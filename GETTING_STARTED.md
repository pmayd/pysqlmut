...
`pyproject.toml` tells pysqlmut where the SQL is and how to run the tests. `command = "uv run python"` is the
...
a `command` directly under `[tool.pysqlmut]`, or `--command`, which runs any test command.
